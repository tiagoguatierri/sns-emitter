...
import { TopicUtils } from './utils/topic-utils.js'
...
export class SNSEmitter {
  ...
  async #runSetup() {
    ...
    await this.#setup.updateConfig(config)
    await this.#listOptions()
  }

  async #showSettings() {
    const config = await this.#setup.getConfig()
    console.log(config)
    await this.#listOptions()
  }
  ...
  async #createTopic() {
    ...
    await this.#topic.add(topic)
    await this.#listOptions()
  }

  async #dispatchTopics() {
    const topics = await this.#topic.list()
  ...
  async #deleteTopics() {
    ...
    const deleted = await this.#topic.delete(...selected.topics)
    if (deleted) {
      console.log(
